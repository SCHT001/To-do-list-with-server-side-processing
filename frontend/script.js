...
function displayTask() {
	...
	if (state.tasks.length > 0) {
		...
		state.tasks.forEach((task) => {
            ...
            <span class='time'>${task.creation_date.hrs}:${task.creation_date.mins
			}</span> </div>
            <div class='mainSide'>
...
//All list display
document.getElementById("allList").addEventListener("click", async () => {
	await getFromServer();
	displayTask();
});

//Pending Display
document.getElementById("pendingList").addEventListener("click", async () => {
	taskList.innerHTML = ``;
	await getFromServer();
	const task = state.tasks;
...
async function completeTask(id) {
	...
	const requestBody = {
		id: id,
		head: state.tasks[index].head,
		description: state.tasks[index].description,
		creation_date: {
			hrs: state.tasks[index].creation_date.hrs,
			mins: state.tasks[index].creation_date.mins,
			weekDay: state.tasks[index].creation_date.weekDay,
		},
		status: 1,
	};
	await fetch(`http://localhost:3000/updateTask/${id}`, {
